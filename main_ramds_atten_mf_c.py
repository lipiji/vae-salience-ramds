...
from vae_attention_mf_cmt import *
import data
...
for topic in f:
    topic = topic.strip('\n')
    print topic, "============"
    i2w, w2i, sents, lines, cmts, csents, w_pos, w_concept,  entities = data.load(base_path + "/docs/" + topic)
    dim_x = len(w2i)
    dim_y = len(w2i)
    num_sents = len(lines)
    num_cmts = len(csents)
    ...
    print "compiling..."
    model = VAE(dim_x, dim_y, hidden_size, latent_size, num_sents, num_cmts, num_summs, optimizer)
    ...
    for i in xrange(500):
        error = 0.0
        in_start = time.time()
        cost, kld, lh, c, d, e, cc, dc, ec, w_summs, Ax = model.train(np.concatenate((sents, cmts), axis=0), lr)
        in_time = time.time() - in_start
        print i, cost, c, d, e, cc, dc, ec
    ...
    #ind = np.argpartition(Xm, -top_w)[-top_w:]
    ind = np.argsort(-Xm)
    ...
    for i in xrange(num_summs):
        s_i = w_summs[i,:]
        #ind = np.argpartition(s_i, -top_w)[-top_w:]
        ind = np.argsort(-s_i)
        for k in xrange(top_w):
            print i2w[ind[k]] + ", ",
        print "\n"
