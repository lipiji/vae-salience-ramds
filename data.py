...
def load(f_path):
    ...
    # load bows of comments
    cmts = np.zeros((num_c, len(w2i)), dtype = theano.config.floatX) 
    f = open(f_path + ".tfcmt", "r")
    i = 0
    for line in f:
        line = line.strip('\n')
        tfs = line.split("|")
        for tf in tfs:
            ...
            if len(fields) < 2:
                #print "Error.tf"
                continue
            cmts[i, w2i[fields[0]]] = float(fields[1])
        i += 1
    f.close()
    ...
    return i2w, w2i, np.asmatrix(sents), lines, np.asmatrix(cmts), csents, w_pos, w_concept, np.asmatrix(entities)
